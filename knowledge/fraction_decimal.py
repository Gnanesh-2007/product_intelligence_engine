import re
from typing import Optional

# 63 exact fractional inch conversions (multiples of 1/64)
DECIMAL_TO_FRACTION = {
    0.015625: "1/64", 0.03125: "1/32", 0.046875: "3/64", 0.0625: "1/16",
    0.078125: "5/64", 0.09375: "3/32", 0.109375: "7/64", 0.125: "1/8",
    0.140625: "9/64", 0.15625: "5/32", 0.171875: "11/64", 0.1875: "3/16",
    0.203125: "13/64", 0.21875: "7/32", 0.234375: "15/64", 0.25: "1/4",
    0.265625: "17/64", 0.28125: "9/32", 0.296875: "19/64", 0.3125: "5/16",
    0.328125: "21/64", 0.34375: "11/32", 0.359375: "23/64", 0.375: "3/8",
    0.390625: "25/64", 0.40625: "13/32", 0.421875: "27/64", 0.4375: "7/16",
    0.453125: "29/64", 0.46875: "15/32", 0.484375: "31/64", 0.5: "1/2",
    0.515625: "33/64", 0.53125: "17/32", 0.546875: "35/64", 0.5625: "9/16",
    0.578125: "37/64", 0.59375: "19/32", 0.609375: "39/64", 0.625: "5/8",
    0.640625: "41/64", 0.65625: "21/32", 0.671875: "43/64", 0.6875: "11/16",
    0.703125: "45/64", 0.71875: "23/32", 0.734375: "47/64", 0.75: "3/4",
    0.765625: "49/64", 0.78125: "25/32", 0.796875: "51/64", 0.8125: "13/16",
    0.828125: "53/64", 0.84375: "27/32", 0.859375: "55/64", 0.875: "7/8",
    0.890625: "57/64", 0.90625: "29/32", 0.921875: "59/64", 0.9375: "15/16",
    0.953125: "61/64", 0.96875: "31/32", 0.984375: "63/64"
}

COMMON_DECIMALS = {
    0.33: "1/3", 0.333: "1/3", 0.67: "2/3", 0.667: "2/3",
    0.1: "1/10", 0.2: "1/5", 0.4: "2/5", 0.6: "3/5", 0.8: "4/5"
}

def decimal_to_fraction(val: float, tolerance: float = 0.005) -> str:
    """Converts a decimal inch measurement to standard fractional form (e.g. 50.25 -> 50-1/4, 0.5 -> 1/2)."""
    integer_part = int(val)
    decimal_part = round(val - integer_part, 6)
    
    if decimal_part < 0.001:
        return str(integer_part)
        
    best_match = None
    min_diff = float("inf")
    for dec_val, frac_str in DECIMAL_TO_FRACTION.items():
        diff = abs(decimal_part - dec_val)
        if diff < min_diff:
            min_diff = diff
            best_match = frac_str
            
    if min_diff <= tolerance and best_match:
        if integer_part == 0:
            return best_match
        return f"{integer_part}-{best_match}"
        
    for dec_val, frac_str in COMMON_DECIMALS.items():
        if abs(decimal_part - dec_val) <= tolerance:
            if integer_part == 0:
                return frac_str
            return f"{integer_part}-{frac_str}"
            
    return str(val)

def convert_string_decimals_to_fractions(text: str) -> str:
    """Converts decimal numbers followed by inch units into trade fraction strings."""
    def repl(match):
        num_str = match.group(1)
        try:
            val = float(num_str)
            frac = decimal_to_fraction(val)
            unit = match.group(2) if match.group(2) else ""
            return f"{frac}{(' ' + unit) if unit else ''}"
        except ValueError:
            return match.group(0)
            
    pattern = r'(\b\d+\.\d+)(?:\s*(in|inch|inches|\"|ft))?'
    return re.sub(pattern, repl, text, flags=re.IGNORECASE)

